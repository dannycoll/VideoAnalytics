function Tree_sum=TreeObjectDetection(Node,Tree,Scale,x,y,IntegralImage,StandardDeviation,InverseArea)
...
check=Node>-1;
if(any(check))
    Tree_sum(check)=TreeObjectDetection(Node(check),Tree,Scale,x(check),y(check),IntegralImage,StandardDeviation(check),InverseArea);
end
