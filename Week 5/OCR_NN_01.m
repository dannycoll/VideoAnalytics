...
%indexesTwos = find (labels == 2);
%indexesThrees = find (labels == 3);
%indexesFours = find (labels == 4);
%indexesFives = find (labels == 5);
%indexesSixes = find (labels == 6);
%indexesSevens = find (labels == 7);
%indexesEights = find (labels == 8);
%indexesNines = find (labels == 9);
...
for k=1:1
    ...
    figure
    title('Correct Classification'+ k)
    count=0;
    i=1;
    ...
    %k=2 provided highest accuracy
    
    %We display 100 of the incorrectly classified images
    figure
    title('Wrong Classification' + k)
    count=0;
    i=1;
    while (count<100)&&(i<=length(comparison))
