...
county = 0;
%The size matrix here sets the size of the window, looping over values for
%I will result in varying window sizes I believe
for r=1:5:size(I,1)
    predictedRow=[];
    county = county + 1;
    countx = 0;
...
%         imshow(digitIm)
        ...
        [prediction, maxi] = SVMDetection(digitIm,modelSVM); %////////////To be completed at STEP 2 \\\\\\\\\\\\  
        if(maxi>1.2) map(county, countx) = maxi; end
